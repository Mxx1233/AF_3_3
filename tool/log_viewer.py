...
class LogViewer:
    def __init__(self, root):
        ...
        tk.Button(top_bar, text="📂 Ordner öffnen", command=self.load_folder, font=("Arial", 11)).pack(side=tk.LEFT, padx=10)
        self.lbl_folder_name = tk.Label(top_bar, text="Kein Ordner geladen", fg="gray")
        self.lbl_folder_name.pack(side=tk.LEFT)

        # Ansichtsumschalter (Radiobuttons)
        tk.Label(top_bar, text="| Ansicht: ", font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=10)
        tk.Radiobutton(top_bar, text="Original", variable=self.view_mode, value="Img_Raw", command=self.refresh_image).pack(side=tk.LEFT)
        tk.Radiobutton(top_bar, text="Debug", variable=self.view_mode, value="Img_Debug", command=self.refresh_image).pack(side=tk.LEFT)
        tk.Radiobutton(top_bar, text="Overlay", variable=self.view_mode, value="Img_Overlay", command=self.refresh_image).pack(side=tk.LEFT)
        ...
        ttk.Separator(self.data_frame, orient='horizontal').pack(fill='x', pady=10)

        self.create_data_field("Lateral_Error", "Abweichung (m):", color="blue", font_size=14)
        self.create_data_field("Heading_Error", "Winkel (rad):")
        self.create_data_field("Curvature", "Krümmung (1/m):")
        
        ttk.Separator(self.data_frame, orient='horizontal').pack(fill='x', pady=10)
        
        self.create_data_field("Steering_Angle", "Lenkbefehl:", color="green", font_size=14)
        ...
        tk.Button(ctrl_bar, text="<< Zurück", command=self.prev_frame).pack(side=tk.LEFT)
        self.slider = tk.Scale(ctrl_bar, from_=0, to=100, orient=tk.HORIZONTAL, command=self.on_slider_drag, showvalue=0)
        self.slider.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=10)
        tk.Button(ctrl_bar, text="Weiter >>", command=self.next_frame).pack(side=tk.LEFT)
    ...
    def create_data_field(self, key, label_text, color="black", font_size=11):
        ...
        tk.Label(frame, text=label_text, font=("Arial", 10), fg="gray", anchor="w").pack(fill=tk.X)
        str_var = tk.StringVar(value="--")
        self.vars[key] = str_var
        tk.Label(frame, textvariable=str_var, font=("Consolas", font_size, "bold"), fg=color, anchor="w").pack(fill=tk.X)
    ...
    def load_folder(self):
        ...
        self.lbl_folder_name.config(text=f"{os.path.basename(folder_path)} ({self.total_frames} Frames)")
    ...
    def update_display(self, index):
        ...
        ts = f"{row_data.get('Timestamp_Sec', 0)}.{row_data.get('Timestamp_Nano', 0)}"
    ...
    def refresh_image(self):
        ...
        if img_name:
            ...
            if os.path.exists(img_path):
                ...
                self.img_label.config(text=f"Datei fehlt: {img_name}", image="")
        else:
