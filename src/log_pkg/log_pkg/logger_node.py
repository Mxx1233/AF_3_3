...
class StreamRecorder(Node):
    def __init__(self):
        ...
        self.get_logger().info('== Aufnahme gestartet ==')
        self.get_logger().info(
            f'Daten werden gespeichert unter: {
                self.save_dir}')
        ...
        sub_overlay = message_filters.Subscriber(
            self, Image, TOPIC_IMAGE_OVERLAY)
        sub_dev = message_filters.Subscriber(
            self, LaneDeviation, TOPIC_DEVIATION)
        ...
        # slop: Erlaubte Zeitabweichung in Sekunden (0.1s ist üblich für Header
        # Propagation)
    ...
    def sync_callback(self, msg_raw, msg_debug, msg_overlay, msg_dev, msg_cmd):
        ...
        try:
            # --- 1. Latenz berechnen ---
            # Differenz zwischen "Jetzt" (Speicherzeitpunkt) und "Header"
            # (Aufnahmezeitpunkt)
            ...
            cv_raw = self.bridge.imgmsg_to_cv2(
                msg_raw, desired_encoding='bgr8')
            cv_debug = self.bridge.imgmsg_to_cv2(
                msg_debug, desired_encoding='bgr8')
            cv_overlay = self.bridge.imgmsg_to_cv2(
                msg_overlay, desired_encoding='bgr8')
            ...
            if self.counter % 10 == 0:
                self.get_logger().info(
                    f'Frame [{
                        self.counter}] gespeichert. Latenz: {
                        latency_ms:.1f}ms')
