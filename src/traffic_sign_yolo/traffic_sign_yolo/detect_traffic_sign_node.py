...
class DetectTrafficSignNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("model_path", "src/traffic_sign_yolo/model/best_run_1_long_30_01_2026.pt")      # Pfad zu deinem .pt
        ...
        self.sub_rgb = message_filters.Subscriber(self, Image, image_topic, qos_profile=qos_profile_sensor_data)
        self.sub_depth = message_filters.Subscriber(self, Image, depth_topic, qos_profile=qos_profile_sensor_data)
    ...
    def on_frames(self, msg_rgb, msg_depth):
        ...
        self.avg_infer_ms = (1.0 - self.alpha) * self.avg_infer_ms + self.alpha * infer_ms
        self.avg_total_ms = (1.0 - self.alpha) * self.avg_total_ms + self.alpha * total_ms

        # Kamera-Latenz berechnen (Zeitpunkt der Aufnahme bis jetzt)
        stamp = msg_rgb.header.stamp.sec + msg_rgb.header.stamp.nanosec * 1e-9
        now = time.time()
        ...
        if now - self.t_last > 1.0:
            ...
            self.get_logger().info(
                f"FPS={fps:.1f} infer={self.avg_infer_ms:.1f}ms total={self.avg_total_ms:.1f}ms cam_latency={lat_ms:.1f}ms"
            )
            self.t_last = now
            self.n_frames = 0
