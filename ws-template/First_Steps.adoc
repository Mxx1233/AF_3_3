...
Diese Anleitung ist mit folgender Version `ucbridge` getestet:
...
Dieser Workspace kann einfach geupdatet werden, indem der aktuelle Stand über git gepullt wird.


=== Update der `ucbridge`

Die `ucbridge` wird über ein Debian-Paket bereitgestellt.

Um zu prüfen, in welcher Version es verwendet wird, kann der Befehl
```
sudo dpkg -l | grep psaf
```
verwendet werden.

Um eine neue Version zu installieren, müssen die alte Pakete über
```
sudo dpkg -r ros-jazzy-psaf-ucbridge
sudo dpkg -r ros-jazzy-psaf-ucbridge-msgs
```
deinstalliert werden.
Eine genaue Version muss hierbei nicht angegeben werden.

Die neuen Pakete werden dann über z.B.
```
sudo dpkg -i ros-jazzy-psaf-ucbridge-msgs_2.3.3-0noble_amd64.deb 
sudo dpkg -i ros-jazzy-psaf-ucbridge_2.3.3-0noble_amd64.deb 
```
...
=== Verwendung des Beispielsnodes `psaf_firststeps`

Als letzte Stufe betrachten wir jetzt die Verwendung des Nodes `psaf_firststeps`, der als Beispiel und Vorlage dient, einen Node, der über Topics mit der `ucbridge` kommuniziert, erstellen kann.
...
Dieser verfügt schon über einen Test, den wir aktivieren können, indem wir in der Datei `src/psaf_firststeps/include/psaf_firststeps/firststeps_node.hpp` die Präprozessor-Konstante `TESTING` auf `true` setzen.
(Dies sollte auch der Fall sein, wenn Sie den Workspace neu geclont haben.)

Danach muss das Package neu gebaut werden:
```
colcon build --symlink-install --packages-select psaf_firststeps
```

Danach kann der Node mit
```
ros2 launch psaf_firststeps firststeps.launch.py
```
gestartet werden.

Es sollten fortlaufend Nachrichten auf der Konsole ausgegeben werden.
Dabei wird für jeden der drei Ultraschallsensoren eine "1" ausgegeben, wenn dieser einen Wert von kleiner als 10cm zurückliefert, ansonsten eine "0".
Außerdem wird der Hinweis angezeigt, dass über das Drücken des Tasters "A" auf dem *ucboard* ein Testzyklus gestartet und gestoppt werden kann.
Drückt man diesen Taster (und lässt ihn dann auch wieder los) sollten sich die Räder zunächst rückwärts (beim ersten Durchlauf der Sequenz kann es ein wenig dauern, bis sich die Räder drehen) und dann vorwärts drehen, und im Anschluss sollte sich die Lenkung von links nach rechts bewegen.
Der Node kann mit CTRL+C abgebrochen werden.
...
+
```
ros2 topic echo --no-arr /camera/realsense2_camera_node/color/image_raw
```
...
*** Camera -> Topic -> `/camera/realsense2_camera_node/color/image_raw`
*** Global Options -> Fixed Frame -> `_color_frame`
-- 
Das Bild der Kamera sollte jetzt links unten angezeigt werden.

* Alternativ gibt es das ROS-Paket `image-view`, welches dazu verwendet werden kann, `image`-Topics anzuzeigen (allerdings etwas langsamer als `rviz2` und mit merklicher Verzögerung):
+
```
ros2 run image_view image_view --ros-args --remap image:=/camera/realsense2_camera_node/color/image_raw
```


== Weitere Schritte

Von hier aus kann man jetzt sich weiter mit der Struktur des `ws-template`-Workspaces vertraut machen.

Auf Dauer ist es ungünstig, immer alle Nodes per Hand in einem eigenen Terminal zu starten.
Hier nehmen die Launch-Files viel Arbeit ab.
Bei dem Workspace ist auch Launch-Files dabei, das alle Nodes zusammen startet:
+
```
ros2 launch psaf_launch main_psaf1.launch.py
```

Während der Entwicklungsphase ist es auch sinnvoll, sich selber passende Launch-Files zu schreiben, die eine für die aktuelle Aufgabe passende Untermenge der Nodes starten.
